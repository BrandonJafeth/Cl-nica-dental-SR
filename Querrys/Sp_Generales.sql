...
-- Procedimiento almacenado para actualizar un registro en la tabla Historial_Med-- Procedimiento almacenado para insertar un registro en la tabla Estado_Pago
CREATE PROCEDURE InsertEstadoPago
    @ID_EstadoPago CHAR(8),
    @Nombre_EP VARCHAR(20),
    @Descripcion_EP VARCHAR(200)
...
-- Procedimiento almacenado para insertar un registro en la tabla Cita
...
AS
BEGIN
    IF @ID_Cita = '' OR @Fecha_Cita IS NULL OR @Motivo = '' OR @Hora_Inicio IS NULL OR @ID_Paciente = '' OR @ID_Dentista = '' OR @ID_Funcionario = '' OR @ID_EstadoCita = ''
    BEGIN
        RAISERROR('No se permiten valores nulos o vacíos', 16, 1);
        RETURN;
    END
    ...
    VALUES (@ID_Cita, @Fecha_Cita, @Motivo, @Hora_Inicio, @ID_Paciente, @ID_Dentista, @ID_Funcionario, @ID_EstadoCita);
END;
GO

-- Procedimiento almacenado para actualizar un registro en la tabla Cita
...
-- Procedimiento almacenado para eliminar un registro de la tabla Cita
...
AS
BEGIN
    IF @ID_Dentista = '' OR @Nombre_Den = '' OR @Apellido1_Den = '' OR @Apellido2_Den = '' OR @Direccion_Den = '' OR @FechaNacimiento_Den IS NULL OR @Telefono_Den = '' OR @Correo_Den = '' OR @ID_Funcionario = ''
    BEGIN
        RAISERROR('No se permiten valores nulos o vacíos', 16, 1);
        RETURN;
    END
...
-- Procedimiento almacenado para insertar un registro en la tabla Auditoria
CREATE PROCEDURE InsertAuditoria
    @ID_Auditoria CHAR(8),
...
AS
BEGIN
    IF @ID_Auditoria = '' OR @Fecha_Hora_Accion IS NULL OR @Descripcion_Accion = '' OR @DispositivoQueRealizo = '' OR @ID_TipoAccion = '' OR @ID_Usuario = ''
    BEGIN
        RAISERROR('No se permiten valores nulos o vacíos', 16, 1);
        RETURN;
   -- Procedimiento almacenado para insertar un registro en la tabla Estado_Citas
...
BEGIN
    ...
    INSERT INTO Auditoria (ID_Auditoria, Fecha_Hora_Accion, Descripcion_Accion, DispositivoQueRealizo, ID_TipoAccion, ID_Usuario)
    VALUES (@ID_Auditoria, @Fecha_Hora_Accion, @Descripcion_Accion, @DispositivoQueRealizo, @ID_TipoAccion, @ID_Usuario);
END;
GO
